...
import torch
import pandas as pd
from utils.anomaly_detection_utils import multivariate_anomaly_detection, univariate_anomaly_detection
...
def test_tadgan(test_loader, encoder, decoder, critic_x, read_path='', signal='', path='', signal_shape=100, params=[]):
        ...
        if params.dataset in ['new_CASAS','CASAS','ELINUS','eHealth']: # type: ignore
            multivariate_anomaly_detection(recons_signal, true_signal, params, combination, critic_score, path)

        else:
            univariate_anomaly_detection(recons_signal, true_signal, params, combination, critic_score, path, read_path, rec_error_type, true_index, known_anomalies, signal, signal_shape)
...
if __name__ == "__main__":
    ...
    if params.hyperbolic: 
        PATH = './trained_models/models_hyper_{}_{}_{}/{}'.format(dataset,str(params.epochs),str(params.lr), params.signal)
    else: 
        PATH = './trained_models/models_eucl_{}_{}_{}/{}'.format(dataset,str(params.epochs),str(params.lr), params.signal)
    ...
    test_tadgan(test_loader, encoder, decoder, critic_x, read_path=read_path,signal = params.signal, path=PATH, signal_shape=params.signal_shape, params=params)
