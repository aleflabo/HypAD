...
def test_tadgan_casas(test_loader, encoder, decoder, critic_x, critic_z, signal = '', hyperbolic = False, path='', signal_shape=100):
    ...
    for batch, (sample,y) in enumerate(test_loader):
    ...
    if len(recons_signal[-1].shape) == 1:
      ...
      if decoder.hyperbolic:
        hyper_recons = np.concatenate(hyper_recons)
        hyper_real = np.concatenate(hyper_real)
        # torch.save(critic_score, 'critic_score.pt')
        torch.save(hyper_recons, path+'recons_hyper.pt')
        torch.save(hyper_real, path+'real_hyper.pt')
      if encoder.hyperbolic:
        hyper_recons_z = np.concatenate(hyper_recons_z)
        torch.save(hyper_recons, path+'hyper_recons_z.pt')

    error, true, pred = score_anomalies(true_signal, recons_signal, critic_score, rec_error_type="point", comb="mult")
...
def score_anomalies(y, y_hat, critic, score_window=10, critic_smooth_window=None,
    ...
    torch.save(critic_scores, '/content/drive/MyDrive/TadGAN/models_eucl/nocturia/critic_score.pt')
...
def reconstruction_errors(y, y_hat, step_size=1, score_window=10, smoothing_window=0.01,
    ...
    for i in range(num_errors):
        intermediate = []
        for j in range(max(0, i - num_errors + pred_length), min(i + 1, pred_length)):
            intermediate.append(abs(y_hat[i - j, j]))
        if intermediate:
...
def find_anomalies(errors, index, z_range=(0, 12), window_size=100, window_size_portion=None,
                   window_step_size=200, window_step_size_portion=None, min_percent=0.13,
...
if __name__ == "__main__":
    ...
    if params.dataset == 'CASAS':
        train_dataset = CasasDataset(path='./data/CASAS/X_tm001_train.pickle',interval=1800)
        test_dataset = CasasDataset(path='./data/CASAS/X_tm001_train.pickle',interval=1800, test=True)
        demo=False
    else:
    ...
    if params.model == 'lstm':
      ...
      if demo:
        test_tadgan(test_loader, encoder, decoder, critic_x, critic_z, known_anomalies, path=path.format(params.signal), signal = params.signal, hyperbolic = params.hyperbolic)
      else:
        test_tadgan(test_loader, encoder, decoder, critic_x, critic_z, known_anomalies, path='./data/{}-test.csv'.format(params.signal),signal = params.signal, hyperbolic = params.hyperbolic)
