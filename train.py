...
def critic_x_iteration(sample, decoder, critic_x, optim_cx, params):
    ...
    z = torch.Tensor(np.random.normal(size=(1, params.batch_size, params.latent_space_dim))).cuda()
...
def decoder_iteration(sample, encoder, decoder, critic_x, critic_z, optim_dec, params, err_loss=nn.MSELoss()):
    optim_dec.zero_grad()
    ...
    z_gen = torch.Tensor(np.random.normal(size=(1, params.batch_size, params.latent_space_dim))).cuda()
...
def train_tadgan(train_loader, encoder, decoder, critic_x, critic_z, n_epochs=2000, params=[], path=""):
    ...
    optim_dec = optim.Adam(list(decoder.parameters())+list(encoder.parameters()), lr=params.lr, betas=(0.9, 0.999))
    if params.hyperbolic:
        optim_dec = geoopt.optim.RiemannianAdam(list(decoder.parameters())+list(encoder.parameters()), lr=params.lr, weight_decay=1e-5, stabilize=10)
    ...
    for epoch in range(n_epochs):
        ...
        for param in critic_x.parameters(): param.requires_grad = True
        for param in critic_z.parameters(): param.requires_grad = True
        ...
        for i in range(n_critics):
            ...
            for batch, sample in enumerate(train_loader):
                loss = critic_x_iteration(sample.cuda(), decoder, critic_x, optim_cx, params)
                cx_loss.append(loss)

                loss = critic_z_iteration(sample.cuda(), encoder, critic_z, optim_cz, params)
                cz_loss.append(loss)
        ...
        for param in decoder.parameters():  param.requires_grad = True
        for param in encoder.parameters():  param.requires_grad = True
        for param in critic_x.parameters(): param.requires_grad = False
        for param in critic_z.parameters(): param.requires_grad = False
        ...
        for batch, sample in enumerate(train_loader):
            ...
            dec_loss, hyper_loss, mse_loss = decoder_iteration(sample.cuda(), encoder, decoder, critic_x, critic_z, optim_dec, params)
        ...
        else: print('Eucl mse loss {}'.format(eucl_dec_loss[-1]))
...
def resume_ckpt(params):
    if params.hyperbolic:
        resume_path = './trained_models/models_hyper_{}_{}/{}/'.format(params.resume_epoch, params.lr, params.signal)
    else:
...
def train(train_loader, params, config_path):
    ...
    encoder = tadgan.Encoder(params.signal_shape, params.latent_space_dim).cuda().train()
    decoder = tadgan.Decoder(params.signal_shape, params.latent_space_dim, params.hyperbolic).cuda().train()
    critic_x = tadgan.CriticX(params.signal_shape, params.latent_space_dim).cuda().train()
    critic_z = tadgan.CriticZ(params.latent_space_dim).cuda().train()
    

    if params.hyperbolic: 
        PATH = './trained_models/models_hyper_{}_{}_{}/{}'.format(params.dataset, str(params.epochs), str(params.lr), params.signal)
    else: 
        PATH = './trained_models/models_eucl_{}_{}_{}/{}'.format(params.dataset, str(params.epochs), str(params.lr), params.signal)
    ...
    TRAIN
    '''
    train_tadgan(train_loader, encoder, decoder, critic_x, critic_z, n_epochs=params.epochs, params=params, path=PATH)
